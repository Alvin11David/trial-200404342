...
const PAYMENT_METHOD_MAP: Record<string, PaymentMethod> = {
  ...
  charge_to_room: "Room Charge",
};

function buildLiveOrders(
  tabs: { id: string; posTableId?: string; orderType: string; deliveryName?: string; status: string; settledBy?: string; openedBy?: string; settlementMethod?: string; paymentMethod?: string; subtotal: number; vatAmount: number; totalAmount: number; settledAt?: string; openedAt: string }[],
  tabItems: { posTabId: string; menuItemId: string; quantity: number; unitPrice: number; isVoided: boolean; isComplimentary: boolean }[],
  menuItems: { id: string; name: string }[],
  tables: { id: string; tableName: string }[],
  users: { id: string; fullName: string }[],
): Order[] {
  const tableName = (id?: string) => tables.find((t) => t.id === id)?.tableName;
  const itemName = (id: string) => menuItems.find((m) => m.id === id)?.name ?? id;
  ...
  for (const t of tabs) {
    ...
    const its = tabItems
      .filter((i) => i.posTabId === t.id && !i.isVoided && !i.isComplimentary)
      .map((i) => ({ name: itemName(i.menuItemId), qty: i.quantity, price: i.unitPrice }));
    ...
    const paymentMethod: PaymentMethod =
      t.settlementMethod === "room_charge"
        ? "Room Charge"
        : (t.paymentMethod && PAYMENT_METHOD_MAP[t.paymentMethod]) || "Cash";
    const table =
      (t.posTableId && tableName(t.posTableId)) ||
      (t.orderType === "delivery" ? t.deliveryName || "Delivery" : t.orderType === "takeaway" ? "Takeaway" : t.orderType === "room_service" ? "Room Service" : "—");
    const dt = t.settledAt ?? t.openedAt;
    const d = new Date(dt);
    ...
    out.push({
      ...
      table,
      cashier: userName(t.settledBy) ?? userName(t.openedBy),
      subtotal: t.subtotal,
      tax: t.vatAmount,
      total: t.totalAmount,
      status,
  ...
  return out.sort((a, b) => (a.dateISO === b.dateISO ? b.time.localeCompare(a.time) : b.dateISO.localeCompare(a.dateISO)));
}
...
const ORDER_TYPE_CONFIG: Record<OrderType, { label: string; icon: typeof ShoppingCart; badge: string }> = {
  dine_in: { label: "Dine In", icon: UtensilsCrossed, badge: "bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-950/30 dark:text-sky-400 dark:border-sky-800" },
  takeaway: { label: "Takeaway", icon: ShoppingCart, badge: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-800" },
  room_service: { label: "Room Service", icon: BedDouble, badge: "bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-950/30 dark:text-violet-400 dark:border-violet-800" },
  delivery: { label: "Delivery", icon: Bike, badge: "bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-950/30 dark:text-orange-400 dark:border-orange-800" },
};
...
function POSOrdersPage() {
  ...
  const storeTables = useStore((s) => s.posTables.filter((t) => t.isActive));
  ...
  const orders = useMemo(
    () => buildLiveOrders(posTabs, posTabItems, menuItems, storeTables, users),
    [posTabs, posTabItems, menuItems, storeTables, users],
  );
  const cashiers = useMemo(() => [...new Set(orders.map((o) => o.cashier))], [orders]);

  const stats = useMemo(() => {
    const today = orders.filter((o) => o.dateISO === businessDate);
  ...
  const filtered = useMemo(
    () =>
      orders.filter((o) => {
        ...
        if (dateFrom && o.dateISO < dateFrom) return false;
        if (dateTo && o.dateISO > dateTo) return false;
        if (query && !`${o.id} ${o.table} ${o.cashier} ${o.items.map(i => i.name).join(" ")}`.toLowerCase().includes(query.toLowerCase()))
          return false;
        return true;
      }),
    [orders, query, statusFilter, cashierFilter, tableFilter, orderTypeFilter, dateFrom, dateTo],
  );
  ...
  return (
    <div className="pos-backoffice-page mx-auto max-w-7xl space-y-8 pb-24">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        {[
          { label: "Today's Orders", value: String(stats.today), gradient: "from-sky-400 to-teal-500", icon: ShoppingCart, detail: "Across all outlets" },
          { label: "Pending", value: String(stats.pending), gradient: "from-amber-400 to-orange-500", icon: Clock, detail: "Awaiting fulfillment" },
          { label: "Completed", value: String(stats.completed), gradient: "from-emerald-400 to-green-500", icon: CheckCircle2, detail: "Settled transactions" },
          { label: "Revenue Today", value: `UGX ${stats.revenue.toLocaleString()}`, gradient: "from-violet-400 to-purple-600", icon: TrendingUp, detail: "Completed orders" },
        ].map((s) => {
          const Icon = s.icon;
          return (
            <div
        ...
        <div className="flex flex-wrap items-center gap-2 p-3">
          ...
          <Select value={tableFilter} onValueChange={setTableFilter}>
            ...
            <SelectContent>
              ...
              {storeTables.map((t) => (
                <SelectItem key={t.id} value={t.tableName}>{t.tableName}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          <Select value={orderTypeFilter} onValueChange={setOrderTypeFilter}>
            ...
            <SelectContent>
              ...
              {(Object.keys(ORDER_TYPE_CONFIG) as OrderType[]).map((ot) => {
                const cfg = ORDER_TYPE_CONFIG[ot];
                const Icon = cfg.icon;
                return (
                  <SelectItem key={ot} value={ot}>
                    <span className="flex items-center gap-2"><Icon className="h-3.5 w-3.5" />{cfg.label}</span>
                  </SelectItem>
                );
              })}
            </SelectContent>
                    ...
                    <div className="flex-1 min-w-0 space-y-3">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground/60">
                        ...
                        <span className={cn(
                          ...
                          ORDER_TYPE_CONFIG[o.orderType]?.badge ?? "",
                        )}>
                          {ORDER_TYPE_CONFIG[o.orderType]?.label ?? o.orderType}
                        </span>
                        <span className="inline-flex items-center gap-1.5">
      ...
      <div className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground/50">
        ...
        {(statusFilter !== "All" || cashierFilter !== "All" || tableFilter !== "All" || orderTypeFilter !== "All" || query || dateFrom || dateTo) && (
          <button
            onClick={() => {
              setQuery("");
              setStatusFilter("All");
