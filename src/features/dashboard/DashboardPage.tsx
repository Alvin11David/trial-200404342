...
function FrontDeskDashboard() {
  ...
  const occ = occupancyOnDate(today);
  const arrivals = reservations.filter((r) => r.checkIn === today && (r.status === "confirmed" || r.status === "open"));
  const departures = reservations.filter((r) => r.checkOut === today && (r.status === "checked_in"));
  const inHouse = reservations.filter((r) => r.status === "checked_in");
  const openFolios = folios.filter((f) => folioBalance(f.id) > 0);
  const totalOutstanding = openFolios.reduce((s, f) => s + folioBalance(f.id), 0);

  const unconfirmed = reservations.filter((r) => r.status === "open");
  const unpaidDeposits = reservations.filter((r) => r.status === "confirmed" && r.folioId && folioBalance(r.folioId) > 0);
  const openRequests = serviceRequests.filter((r) => r.status === "open" || r.status === "in_progress");
  ...
  return (
    <>
      <div className="flex flex-wrap gap-2">
        ...
        <Link to="/guests?tab=in-house" className="dashboard-cta inline-flex items-center gap-1.5 rounded-xl bg-warning/10 px-4 py-2 text-xs font-semibold text-warning"><LogOut className="h-3.5 w-3.5" /> Check Out <ArrowRight className="h-3 w-3" /></Link>
        ...
        <Link to="/billing" className="dashboard-cta inline-flex items-center gap-1.5 rounded-xl bg-info/10 px-4 py-2 text-xs font-semibold text-info"><CreditCard className="h-3.5 w-3.5" /> Record Payment <ArrowRight className="h-3 w-3" /></Link>
        <Link to="/guests" className="dashboard-cta inline-flex items-center gap-1.5 rounded-xl bg-secondary/10 px-4 py-2 text-xs font-semibold text-secondary-foreground"><Search className="h-3.5 w-3.5" /> Search Guest <ArrowRight className="h-3 w-3" /></Link>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="animate-kpi-enter"><KpiCard label="Arrivals today" value={String(arrivals.length)} delta={`${(occ.pct * 100).toFixed(0)}% occ`} icon={<CalendarCheck2 className="h-4 w-4" />} accent="primary" index={0} /></div>
        <div className="animate-kpi-enter" style={{ animationDelay: "80ms" }}><KpiCard label="Departures today" value={String(departures.length)} delta={`${inHouse.length} in-house`} icon={<CalendarX2 className="h-4 w-4" />} accent="warning" index={1} /></div>
        <div className="animate-kpi-enter" style={{ animationDelay: "160ms" }}><KpiCard label="In-house guests" value={String(inHouse.length)} delta={`${(occ.pct * 100).toFixed(0)}% occ`} deltaPositive icon={<Users className="h-4 w-4" />} accent="info" index={2} /></div>
        <div className="animate-kpi-enter" style={{ animationDelay: "240ms" }}><KpiCard label="Open folios" value={String(openFolios.length)} delta={fmtUGX(totalOutstanding)} icon={<Receipt className="h-4 w-4" />} accent="success" index={3} /></div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Arrivals" action={<Link to="/check-in" className="text-xs font-medium text-primary hover:text-primary/80 transition-colors">View all &rarr;</Link>} accent="primary">
          <GuestTable
            rows={arrivals.slice(0, 5).map((r) => ({
              name: r.guestName,
              room: r.roomId ?? "\u2014",
        ...
        <Card title="Departures" action={<Link to="/guests?tab=in-house" className="text-xs font-medium text-primary hover:text-primary/80 transition-colors">View all &rarr;</Link>} accent="warning">
          <GuestTable
            rows={departures.slice(0, 5).map((r) => ({
              name: r.guestName,
              room: r.roomId ?? "\u2014",
              time: "\u2014",
              nights: nightsBetween(r.checkIn, r.checkOut),
              status: r.folioId && folioBalance(r.folioId) > 0 ? "Folio open" : "Cleared",
            }))}
            kind="departure"
          />
        </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Pending tasks" subtitle={`${unconfirmed.length + unpaidDeposits.length + openRequests.length} items`} accent="warning">
          <div className="space-y-3">
            {unconfirmed.length > 0 && (
              <div>
                <h4 className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-muted-foreground">
                  <CalendarX2 className="h-3.5 w-3.5 text-warning" />
                  Unconfirmed reservations
                </h4>
                <div className="space-y-1">
                  {unconfirmed.slice(0, 4).map((r) => (
                    <Link
                      key={r.id}
                      to={`/reservations/${r.id}`}
                      className="flex items-center justify-between rounded-lg border border-border/40 bg-card/30 px-3 py-2 text-xs transition hover:border-warning/30 hover:bg-warning/5"
                    >
                  ...
                  {unconfirmed.length > 4 && (
                    <Link to="/reservations" className="block text-center text-[10px] font-medium text-warning transition hover:text-warning/80">
                      +{unconfirmed.length - 4} more
                    </Link>
                  )}
                </div>
              </div>
            ...
            {unpaidDeposits.length > 0 && (
              <div>
                ...
                <div className="space-y-1">
                  {unpaidDeposits.slice(0, 4).map((r) => (
                    <Link
                      key={r.id}
                      to={`/billing?folio=${r.folioId}`}
                      className="flex items-center justify-between rounded-lg border border-border/40 bg-card/30 px-3 py-2 text-xs transition hover:border-destructive/30 hover:bg-destructive/5"
                    >
                      <span className="font-medium">{r.guestName}</span>
                      <span className="font-semibold text-destructive tabular-nums">{fmtUGX(folioBalance(r.folioId!))}</span>
                    </Link>
                  ))}
                  {unpaidDeposits.length > 4 && (
                    <Link to="/billing" className="block text-center text-[10px] font-medium text-destructive transition hover:text-destructive/80">
                      +{unpaidDeposits.length - 4} more
                    </Link>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {openRequests.slice(0, 4).map((r) => (
                    <div
                      key={r.id}
                      className="flex items-center justify-between rounded-lg border border-border/40 bg-card/30 px-3 py-2 text-xs"
                    >
                      <div className="min-w-0 flex-1">
                        <span className="font-medium">Room {r.roomId}</span>
                        <p className="truncate text-muted-foreground">{r.description}</p>
                      </div>
                      <span className="ml-2 shrink-0 rounded-md bg-info/10 px-1.5 py-0.5 text-[10px] font-medium text-info">{r.urgency}</span>
                    </div>
                  ))}
                  {openRequests.length > 4 && (
                    <p className="text-center text-[10px] text-muted-foreground">+{openRequests.length - 4} more</p>
                  )}
                </div>
              </div>
            )}
            {unconfirmed.length === 0 && unpaidDeposits.length === 0 && openRequests.length === 0 && (
              <div className="flex flex-col items-center py-6 text-center">
                <ListTodo className="mb-2 h-6 w-6 text-success/60" />
                <p className="text-xs text-muted-foreground">All clear — no pending tasks.</p>
              </div>
            )}
          </div>
        </Card>

        <Card title="Rooms needing attention" subtitle={`${dirtyRooms.length + maintRooms.length + cleanRooms.length} rooms`} action={<Link to="/rooms" className="text-xs font-medium text-primary hover:text-primary/80 transition-colors">View all &rarr;</Link>} accent="primary">
          <div className="space-y-3">
            {dirtyRooms.length > 0 && (
              <div>
                <h4 className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-muted-foreground">
                  <Sparkles className="h-3.5 w-3.5 text-warning" />
                  Dirty — {dirtyRooms.length} room{dirtyRooms.length > 1 ? "s" : ""}
                </h4>
                <div className="flex flex-wrap gap-1.5">
                  {dirtyRooms.slice(0, 8).map((r) => (
                    <Link
                      key={r.id}
                      to="/rooms"
                      className="inline-flex items-center gap-1 rounded-lg border border-warning/20 bg-warning/5 px-2.5 py-1 text-xs font-medium text-warning transition hover:border-warning/40 hover:bg-warning/10"
                    >
                  ...
                  {dirtyRooms.length > 8 && (
                    <span className="inline-flex items-center text-[10px] text-muted-foreground">+{dirtyRooms.length - 8}</span>
                  )}
                </div>
              </div>
            )}
            {maintRooms.length > 0 && (
              <div>
                <h4 className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-muted-foreground">
                  ...
                  Out of order — {maintRooms.length} room{maintRooms.length > 1 ? "s" : ""}
                </h4>
                <div className="flex flex-wrap gap-1.5">
                  {maintRooms.slice(0, 8).map((r) => (
                    <Link
                      key={r.id}
                      to="/rooms"
                      className="inline-flex items-center gap-1 rounded-lg border border-destructive/20 bg-destructive/5 px-2.5 py-1 text-xs font-medium text-destructive transition hover:border-destructive/40 hover:bg-destructive/10"
                    >
                      <BedDouble className="h-3 w-3" />
                      {r.id}
                    </Link>
                  ...
                  {maintRooms.length > 8 && (
                    <span className="inline-flex items-center text-[10px] text-muted-foreground">+{maintRooms.length - 8}</span>
                  )}
                </div>
              </div>
            )}
            {cleanRooms.length > 0 && (
              <div>
                <h4 className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-muted-foreground">
                  ...
                  Due for inspection — {cleanRooms.length} room{cleanRooms.length > 1 ? "s" : ""}
                </h4>
                <div className="flex flex-wrap gap-1.5">
                  {cleanRooms.slice(0, 8).map((r) => (
                    <Link
                      key={r.id}
                      to="/rooms"
                      className="inline-flex items-center gap-1 rounded-lg border border-info/20 bg-info/5 px-2.5 py-1 text-xs font-medium text-info transition hover:border-info/40 hover:bg-info/10"
                    >
                      <BedDouble className="h-3 w-3" />
                      {r.id}
                    </Link>
                  ...
                  {cleanRooms.length > 8 && (
                    <span className="inline-flex items-center text-[10px] text-muted-foreground">+{cleanRooms.length - 8}</span>
                  )}
                </div>
              </div>
            )}
            {dirtyRooms.length === 0 && maintRooms.length === 0 && cleanRooms.length === 0 && (
              <div className="flex flex-col items-center py-6 text-center">
                <Building2 className="mb-2 h-6 w-6 text-success/60" />
                <p className="text-xs text-muted-foreground">All rooms in good shape.</p>
              </div>
            )}
          </div>
        </Card>
        ...
        <Card title="Hourly Sales" subtitle={`Today \u00b7 ${fmt(total)} total`} className="lg:col-span-2" accent="primary">
          <ChartContainer config={{ sales: { label: "Sales", color: "var(--color-primary)" } }} className="h-44 w-full">
            <AreaChart data={hourlyData} margin={{ top: 12, right: 12, left: -20, bottom: 4 }}>
              ...
              <ChartTooltip cursor={false} content={<ChartTooltipContent indicator="dot" className="rounded-xl border-border/50 shadow-lg backdrop-blur-xl" formatter={(v) => typeof v === "number" ? fmt(v) : ""} />} />
...
function AccountantDashboard() {
  ...
  return (
    <>
      <div className="flex flex-wrap gap-2">
        <Link to="/billing" className="dashboard-cta inline-flex items-center gap-1.5 rounded-xl bg-primary/10 px-4 py-2 text-xs font-semibold text-primary"><Receipt className="h-3.5 w-3.5" /> View Billing <ArrowRight className="h-3 w-3" /></Link>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card title="Today's payments" accent="info">
          <div className="flex flex-col items-center justify-center py-6 text-center">
            ...
            <Link to="/billing" className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-0.5">Open Billing <ArrowRight className="h-3 w-3" /></Link>
          </div>
        </Card>
      </div>
    </>
