...
const initialTasks: Task[] = [
  {
    ...
    room: "204",
    floor: 2,
    type: "Turnover after checkout",
    priority: "High",
    status: "Queued",
    assignee: "Joan Nansubuga",
    due: "11:30",
...
function HousekeepingPage() {
  ...
  const filtered = useMemo(() => {
    return tasks.filter((t) => {
      if (
        query &&
        !`${t.room} ${t.type} ${t.assignee ?? ""}`.toLowerCase().includes(query.toLowerCase())
      )
        return false;
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Live operations board for the housekeeping team.
        </p>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="col-span-3">
                  ...
                  <span
                    className={cn(
                      "mt-1.5 inline-flex rounded-full border px-2 py-0.5 text-[10px] font-medium",
                      s.bg,
                      s.color,
                    )}
                  >
...
function StatTile({
  ...
  return (
    <div className="glass card-hover relative overflow-hidden rounded-2xl p-5">
      <div
        className={cn(
          "absolute -right-10 -top-10 h-28 w-28 rounded-full opacity-25 blur-2xl bg-gradient-to-br",
          grad[tone],
        )}
      />
      <div className="relative flex items-center gap-4">
        <span
          className={cn(
            "grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br text-primary-foreground",
            grad[tone],
          )}
        >
