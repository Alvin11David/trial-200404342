...
async function run() {
  ...
  try {
    ...
    // ===== 5. Initiate MTN MoMo payment (creates as Pending) =====
    console.log("\n=== 5. Initiate MTN MoMo payment ===");
    ...
    // Select MTN MoMo
    await payDialog.locator("label").filter({ hasText: "Method" }).locator("select").selectOption("mtn_momo");

    // Enter amount
    const momoAmount = Math.min(30000, initialBalance);
    await payDialog.locator("label").filter({ hasText: "Amount (UGX)" }).locator("input").fill(String(momoAmount));

    // Enter phone number
    await payDialog.locator("label").filter({ hasText: "Mobile money phone" }).locator("input").fill("+256 772 123 456");
    await ss("mf05-momo-filled");
    ...
    await page.waitForTimeout(1500);
    await ss("mf06-after-momo-payment");
    ...
    ok(bodyText.includes("MTN Mobile Money"), "Payment shows 'MTN Mobile Money' method");
    ...
    const expectedAfterConfirm = initialBalance - momoAmount;
    ...
    ok(totalPayAfterConfirm >= initialTotalPay + momoAmount,
      `Total payments increased to include MoMo: UGX ${totalPayAfterConfirm.toLocaleString()}`);
    ...
    // ===== 9. Initiate second MoMo payment and fail it =====
    console.log("\n=== 9. Initiate MoMo payment and fail it ===");
    await page.locator("button:has-text('Record payment')").click();
    await page.waitForTimeout(800);
    await ss("mf08-second-momo");
    ...
    await payDialog2.locator("label").filter({ hasText: "Amount (UGX)" }).locator("input").fill("10000");
    await payDialog2.locator("label").filter({ hasText: "Mobile money phone" }).locator("input").fill("+256 700 111 222");
    await page.locator("button:has-text('Record payment')").last().click();
    await page.waitForTimeout(1500);
    await ss("mf09-second-pending");
    ...
    // Handle the prompt dialog for failure reason
    page.once("dialog", (dialog) => {
      dialog.accept("Customer cancelled the payment");
    });
    await failBtn.click();
    await page.waitForTimeout(1500);
    await ss("mf10-after-fail");
    ...
    ok(bodyAfterFail.includes("Customer cancelled"), "Failure reason displayed");
    ...
    ok(auditBody.includes("Customer cancelled"), "Audit contains failure reason");

  } catch (err) {
    console.log(`\n  ERROR: ${err.message}`);
    console.log(err.stack.split("\n").slice(0, 5).join("\n"));
    await ss("mf-error").catch(() => {});
    no(err.message);
  }
